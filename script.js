...
`;
  ...
  document.addEventListener("click", (e) => {
    if (e.target.classList.contains("addTripBtn")) {
      ...
      if (!card) return;
      const name = card.querySelector("h4") ? card.querySelector("h4").textContent : "Unknown";
      const desc = card.querySelector(".desc") ? card.querySelector(".desc").textContent : "";
      const img = card.querySelector("img") ? card.querySelector("img").src : "";
      const mapLink = card.querySelector(".map-link") ? card.querySelector(".map-link").href : "";

      const trip = { name, description: desc, image: img, map: mapLink };
  ...
  function showMyTrips() {
    ...
    trips.forEach(trip => {
      ...
      card.innerHTML = `
        ...
        <p><a href="${trip.map}" target="_blank" class="map-link">📍 View on Map</a></p>
      `;
      // style card image fallback
      const img = card.querySelector("img");
      if (img) {
        img.style.width = "100%";
        img.style.height = "200px";
